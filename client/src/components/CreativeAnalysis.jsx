...
const ChatMessage = ({ message, isStreaming }) => {
  ...
  const renderContent = () => {
    ...
    // For streaming messages: ONLY show completed sentences, hide pending text
    const sentences = message.sentences || [];
    
    return (
      <div className="ci-markdown">
        {sentences.length === 0 ? (
          // Show thinking indicator when no sentences yet
          ...
          <>
            ...
            {/* Only show cursor while still streaming, no pendingText visible */}
            <span className="ci-cursor" />
          </>
        )}
      </div>
...
export default function CreativeIntelligence({ store }) {
  ...
  // FIX #2: Show scroll button when content below viewport (not just during loading)
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      const hasContentBelow = container.scrollHeight - container.scrollTop - container.clientHeight > 50;
  ...
  // FIX #1 & #4: Auto-scroll logic - respects user scroll position and settings
  useEffect(() => {
    // If auto-scroll is OFF, never auto-scroll
    if (settings?.autoScroll === false) return;
    
    // If user has scrolled up, don't yank them back down
    if (isUserScrolledUp()) return;
    
    // Only auto-scroll if we're near bottom
    if (!checkIfAtBottom()) return;
    
    // Scroll to bottom
    scrollToBottom(true);
  }, [chatMessages, settings?.autoScroll, scrollToBottom, checkIfAtBottom, isUserScrolledUp]);

  // Remove the "scroll when streaming completes" behavior when auto-scroll is OFF
  ...
  const handleSendMessage = async (msgText) => {
    ...
    // FIX #5: Only reset scroll position if auto-scroll is ON
