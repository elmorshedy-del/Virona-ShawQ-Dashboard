...
function getCampaignDailyMetrics(campaign, periodDays) {
  const daysActive = Number(campaign?.daysActive || 0);
  const days = Math.max(daysActive || periodDays || 1, 1);
...
overhead=${inputs.overhead || '0'}`;
  ...
  const campaignContext = mode === 'all'
    ? `All Campaigns (Unified data across campaigns, ${periodDays || 'n/a'} days):
${allCampaigns}`
    : `Campaign A:
${campaignA}
...
export default function BudgetCalculator({ campaigns = [], periodDays = 30, storeName = '' }) {
  ...
  const campaignOptions = useMemo(() => {
    return campaigns
      ...
      .map((campaign) => ({
        ...
        daysActive: campaign.daysActive || 0
      }))
  ...
  const applyCampaignInputs = () => {
    ...
    const getGroupDailyTotals = (group) => {
      return group.reduce((acc, campaign) => {
        const metrics = getCampaignDailyMetrics(campaign, days);
        acc.spendDaily += metrics.spendDaily;
        acc.purchasesDaily += metrics.purchasesDaily;
        return acc;
      }, { spendDaily: 0, purchasesDaily: 0 });
    };

    const highDailyTotals = getGroupDailyTotals(highSpendGroup);
    const lowDailyTotals = getGroupDailyTotals(lowSpendGroup);
  ...
  const allCampaignContext = useMemo(() => {
    ...
    return campaignOptions.map((campaign) => {
      ...
      const daySource = campaign.daysActive ? `${campaign.daysActive} days active` : `${periodDays || 'n/a'} days`;
      return `• ${campaign.name} (${campaign.countries || '—'}): spend ${formatCurrency(daily.spendDaily)}/day, purchases ${formatNumber(daily.purchasesDaily)}, revenue ${formatCurrency(daily.revenue)} (${daySource})`;
    }).join('\n');
  ...
  const campaignAContext = useMemo(() => {
    ...
    const daySource = selectedCampaignA.daysActive ? `${selectedCampaignA.daysActive} days active` : `${periodDays || 'n/a'} days`;
    return `${selectedCampaignA.name} (${selectedCampaignA.countries || '—'}): spend ${formatCurrency(daily.spendDaily)}/day, purchases ${formatNumber(daily.purchasesDaily)}, revenue ${formatCurrency(daily.revenue)} (${daySource})`;
  ...
  const campaignBContext = useMemo(() => {
    ...
    const daySource = selectedCampaignB.daysActive ? `${selectedCampaignB.daysActive} days active` : `${periodDays || 'n/a'} days`;
    return `${selectedCampaignB.name} (${selectedCampaignB.countries || '—'}): spend ${formatCurrency(daily.spendDaily)}/day, purchases ${formatNumber(daily.purchasesDaily)}, revenue ${formatCurrency(daily.revenue)} (${daySource})`;
  }, [selectedCampaignB, periodDays]);

  const handleGptSubmit = async (event, customQuery = null) => {
    event?.preventDefault();
    const request = customQuery ?? gptQuery;
    if (!request.trim() || gptLoading) return;
  ...
  const handleGptFollowupSubmit = async (event) => {
    event?.preventDefault();
    if (!gptFollowup.trim() || gptLoading) return;
    const followup = gptFollowup.trim();
    setGptQuery(followup);
    setGptFollowup('');
    await handleGptSubmit(null, followup);
  };
        ...
        <div className="p-6 space-y-4">
          ...
          {(gptStreamingText || gptResult) && (
            <div className="rounded-2xl border border-purple-100 bg-purple-50/60 p-4">
              ...
              {!gptLoading && (
                <form onSubmit={handleGptFollowupSubmit} className="mt-4">
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="Ask a follow-up about this calculation..."
                      className="w-full rounded-xl border border-purple-100 bg-white/80 pl-10 pr-12 py-2.5 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-purple-300"
                    />
                    <button
                      type="submit"
                      disabled={!gptFollowup.trim()}
                      className="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg bg-purple-600 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-purple-700 disabled:bg-gray-300"
                    >
                      Send
                    </button>
                  </div>
