...
overhead=${inputs.overhead || '0'}`;
  ...
  const countryContext = countryA && countryB
    ? `Country A: ${countryA}\nCountry B: ${countryB}`
    : '';

  const campaignContext = mode === 'all'
    ? `All Campaigns (Unified data across campaigns with per-campaign run days, compare Country A vs Country B):
${allCampaigns}`
    : `Campaign A:
${campaignA}
...
export default function BudgetCalculator({
  ...
  const campaignOptions = useMemo(() => {
    return campaignCountries
      .filter((campaign) => campaign?.campaignId && campaign?.country)
      .map((campaign) => ({
  ...
  const filteredCampaignAOptions = useMemo(() => {
    return campaignOptions.filter((campaign) => campaign.country === countryA);
  }, [campaignOptions, countryA]);

  const filteredCampaignBOptions = useMemo(() => {
    return campaignOptions.filter((campaign) => campaign.country === countryB);
  }, [campaignOptions, countryB]);

  const selectedCampaignA = useMemo(
    () => campaignOptions.find((campaign) => campaign.id === campaignAId && campaign.country === countryA),
    [campaignAId, campaignOptions, countryA]
  );
  const selectedCampaignB = useMemo(
    () => campaignOptions.find((campaign) => campaign.id === campaignBId && campaign.country === countryB),
    [campaignAId, campaignBId, campaignOptions, countryB]
  );
  ...
  const applyCampaignInputs = () => {
    ...
    if (campaignMode === 'two') {
      if (!selectedCampaignA || !selectedCampaignB) {
        setCampaignError('Select two campaigns (each with a country) to build the calculator points.');
        return;
      }

      if (countryA && countryB && countryA === countryB) {
        setCampaignError('Country A and Country B must be different for a country comparison.');
        return;
      }

      if (selectedCampaignA.id === selectedCampaignB.id && selectedCampaignA.country === selectedCampaignB.country) {
    ...
    if (!countryA || !countryB) {
      setCampaignError('Select Country A and Country B for the unified comparison.');
      return;
    }
    ...
    const countryAOptions = campaignOptions.filter((campaign) => campaign.country === countryA);
    const countryBOptions = campaignOptions.filter((campaign) => campaign.country === countryB);

    if (!countryAOptions.length || !countryBOptions.length) {
      setCampaignError('Selected countries need campaign data to build the model.');
      return;
    }

    const countryATotals = sumCampaignMetrics(countryAOptions);
    const countryBTotals = sumCampaignMetrics(countryBOptions);

    const countryADailySpend = countryAOptions.reduce(
      (acc, campaign) => acc + getCampaignDailyMetrics(campaign, periodDays).spendDaily,
      0
    );
    const countryADailyPurchases = countryAOptions.reduce(
      (acc, campaign) => acc + getCampaignDailyMetrics(campaign, periodDays).purchasesDaily,
      0
    );
    const countryBDailySpend = countryBOptions.reduce(
      (acc, campaign) => acc + getCampaignDailyMetrics(campaign, periodDays).spendDaily,
      0
    );
    const countryBDailyPurchases = countryBOptions.reduce(
      (acc, campaign) => acc + getCampaignDailyMetrics(campaign, periodDays).purchasesDaily,
      0
    );

    if (!countryADailySpend || !countryADailyPurchases || !countryBDailySpend || !countryBDailyPurchases) {
      setCampaignError('Unified campaigns need spend and conversions to build the model.');
      return;
    }

    const totalPurchases = countryATotals.purchases + countryBTotals.purchases;
    const totalRevenue = countryATotals.revenue + countryBTotals.revenue;
    const aov = totalPurchases > 0 ? totalRevenue / totalPurchases : 0;

    setInputs((prev) => ({
      ...prev,
      spend1: countryADailySpend.toFixed(2),
      conv1: countryADailyPurchases.toFixed(2),
      spend2: countryBDailySpend.toFixed(2),
      conv2: countryBDailyPurchases.toFixed(2),
      aov: aov > 0 ? aov.toFixed(2) : prev.aov
    }));
  };
  ...
  const allCampaignContext = useMemo(() => {
    ...
    return campaignOptions.map((campaign) => {
      ...
      return `• ${campaign.name} (${campaign.country || '—'}): spend ${formatCurrency(daily.spendDaily)}/day over ${daily.daysRunning} days, purchases ${formatNumber(daily.purchasesDaily)}, revenue ${formatCurrency(daily.revenue)}`;
    }).join('\n');
  ...
  const campaignAContext = useMemo(() => {
    ...
    return `${selectedCampaignA.name} (${selectedCampaignA.country || '—'}): spend ${formatCurrency(daily.spendDaily)}/day over ${daily.daysRunning} days, purchases ${formatNumber(daily.purchasesDaily)}, revenue ${formatCurrency(daily.revenue)}`;
  ...
  const campaignBContext = useMemo(() => {
    ...
    return `${selectedCampaignB.name} (${selectedCampaignB.country || '—'}): spend ${formatCurrency(daily.spendDaily)}/day over ${daily.daysRunning} days, purchases ${formatNumber(daily.purchasesDaily)}, revenue ${formatCurrency(daily.revenue)}`;
          ...
          {campaignMode === 'two' && (
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2 md:col-span-2">
                <label className="text-sm font-medium text-gray-600">Country A vs Country B</label>
                <div className="grid gap-3 sm:grid-cols-2">
                  <select
                    value={countryA}
                    onChange={(event) => {
                      setCountryA(event.target.value);
                      setCampaignAId('');
                    }}
                  ...
                  <select
                    value={countryB}
                    onChange={(event) => {
                      setCountryB(event.target.value);
                      setCampaignBId('');
                    }}
                    className="w-full rounded-lg border border-gray-200 px-3 py-2 text-base focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  >
                ...
                <p className="text-xs text-gray-400">Comparisons are always country vs country.</p>
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-600">Campaign A (Country A)</label>
                <select
                  value={campaignAId}
                ...
                >
                  ...
                  {filteredCampaignAOptions.map((campaign) => (
                    <option key={`${campaign.id}-${campaign.country}`} value={campaign.id}>
                      {campaign.name}
                    </option>
                  ))}
                </select>
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-600">Campaign B (Country B)</label>
                <select
                  value={campaignBId}
                ...
                >
                  <option value="">Select campaign</option>
                  {filteredCampaignBOptions.map((campaign) => (
                    <option key={`${campaign.id}-${campaign.country}`} value={campaign.id}>
                      {campaign.name}
                    </option>
                  ))}
          ...
          {campaignMode === 'all' && (
            <div className="space-y-3">
              ...
              <div className="rounded-xl border border-indigo-100 bg-indigo-50/60 p-4 text-sm text-indigo-700">
                Unified campaigns are aggregated per country so the calculator compares Country A vs Country B.
              </div>
            </div>
          )}
