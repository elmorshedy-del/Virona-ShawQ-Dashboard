...
function getCampaignDailyMetrics(campaign, periodDays) {
  const days = Math.max(campaign?.daysRunning || periodDays || 1, 1);
...
function buildCampaignPrompt({ mode, campaignA, campaignB, allCampaigns, periodDays, inputs, request }) {
  const header = `You are a budget calculator assistant. Use the data below to compute the two-point model exactly like the calculator. Daily spend is total spend divided by days running when provided.`;
...
overhead=${inputs.overhead || '0'}`;
  ...
  const campaignContext = mode === 'all'
    ? `All Campaigns (Unified data across campaigns, use per-campaign days when provided; fallback ${periodDays || 'n/a'} days):
${allCampaigns}`
    : `Campaign A:
${campaignA}
...
export default function BudgetCalculator({ campaigns = [], periodDays = 30, storeName = '' }) {
  ...
  const campaignOptions = useMemo(() => {
    return campaigns
      ...
      .map((campaign) => ({
        ...
        daysRunning: campaign.daysRunning || periodDays || 1
      }))
  ...
  const applyCampaignInputs = () => {
    setCampaignError('');
    const defaultDays = Math.max(periodDays || 1, 1);
    ...
    if (campaignMode === 'two') {
      ...
      const campaignA = getCampaignDailyMetrics(selectedCampaignA, defaultDays);
      const campaignB = getCampaignDailyMetrics(selectedCampaignB, defaultDays);
    ...
    const highDays = Math.max(highSpendGroup.reduce((acc, campaign) => acc + (campaign.daysRunning || defaultDays), 0), 1);
    const lowDays = Math.max(lowSpendGroup.reduce((acc, campaign) => acc + (campaign.daysRunning || defaultDays), 0), 1);

    const highDailySpend = highTotals.spend / highDays;
    const highDailyPurchases = highTotals.purchases / highDays;
    const lowDailySpend = lowTotals.spend / lowDays;
    const lowDailyPurchases = lowTotals.purchases / lowDays;
  ...
  const allCampaignContext = useMemo(() => {
    ...
    return campaignOptions.map((campaign) => {
      ...
      return `• ${campaign.name} (${campaign.countries || '—'}): spend ${formatCurrency(daily.spendDaily)}/day, purchases ${formatNumber(daily.purchasesDaily)}, revenue ${formatCurrency(daily.revenue)}, days ${daily.daysRunning}`;
    }).join('\n');
  ...
  const campaignAContext = useMemo(() => {
    ...
    return `${selectedCampaignA.name} (${selectedCampaignA.countries || '—'}): spend ${formatCurrency(daily.spendDaily)}/day, purchases ${formatNumber(daily.purchasesDaily)}, revenue ${formatCurrency(daily.revenue)}, days ${daily.daysRunning}`;
  ...
  const campaignBContext = useMemo(() => {
    ...
    return `${selectedCampaignB.name} (${selectedCampaignB.countries || '—'}): spend ${formatCurrency(daily.spendDaily)}/day, purchases ${formatNumber(daily.purchasesDaily)}, revenue ${formatCurrency(daily.revenue)}, days ${daily.daysRunning}`;
    ...
    <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                {selectedCampaignA && (
                  <p className="text-xs text-gray-400">
                    ...
                    {` ${getCampaignDailyMetrics(selectedCampaignA, periodDays).daysRunning} days`}
                  </p>
                )}
              </div>
              <div className="space-y-2">
                ...
                {selectedCampaignB && (
                  <p className="text-xs text-gray-400">
                    ...
                    {` ${getCampaignDailyMetrics(selectedCampaignB, periodDays).daysRunning} days`}
                  </p>
                )}
              </div>
            </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200">
        ...
        <div className="p-6 space-y-4">
          ...
          {(gptResult || gptStreamingText) && (
            <form onSubmit={handleGptSubmit} className="space-y-2">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-purple-400"
                />
                <button
                  type="submit"
                  disabled={gptLoading || !gptQuery.trim()}
                  className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md bg-purple-600 px-3 py-1 text-xs font-semibold text-white transition hover:bg-purple-700 disabled:bg-gray-300"
                >
                  Send →
                </button>
              </div>
