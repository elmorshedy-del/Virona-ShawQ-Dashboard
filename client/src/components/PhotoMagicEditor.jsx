      ...
      {options.map((opt) => (
        <button
          ...
          className={cn(
            ...
            'disabled:opacity-50 disabled:cursor-not-allowed',
            value === opt.value ? 'border-violet-200 bg-violet-50 text-violet-700' : 'border-gray-200 bg-white text-gray-700 hover:bg-gray-50'
          )}
          title={opt.title}
        >
          {opt.label}
...
export default function PhotoMagicEditor({ store }) {
  ...
  const aiOk = Boolean(health?.photo_magic?.ai?.health?.ok);
  const aiModels = health?.photo_magic?.ai?.health?.payload?.models || {};
  ...
  const onMaskPointerDown = useCallback((e) => {
  ...
  const onMaskPointerMove = useCallback((e) => {
    if (tool !== 'erase') return;
    const canvas = maskCanvasRef.current;
    if (!canvas) return;
  ...
  const onMaskPointerUp = useCallback((e) => {
    if (tool !== 'erase') return;
    const canvas = maskCanvasRef.current;
    if (!canvas) return;
  ...
  }, [cropMarginPx, cropToMask, eraseMode, exportMaskBase64, imageId, maskDilatePx, maskFeatherPx, maxSide, quality, refreshHealth, sdxlGuidance, sdxlSeed, sdxlSteps, sdxlStrength, store]);
        ...
        <div className="mt-4 grid grid-cols-1 lg:grid-cols-[420px_1fr_420px] gap-4 items-start">
          ...
          <div className="rounded-3xl border border-gray-200 bg-white shadow-sm p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <StatusPill ok={Boolean(aiOk && aiModels?.rmbg2)} label="RMBG2" title={health?.photo_magic?.ai?.health?.payload?.errors?.rmbg2 || ''} />
              <StatusPill ok={Boolean(aiOk && aiModels?.sam2)} label="SAM2" title={health?.photo_magic?.ai?.health?.payload?.errors?.sam2 || ''} />
              <StatusPill ok={Boolean(aiOk && aiModels?.lama)} label="LaMa" title={health?.photo_magic?.ai?.health?.payload?.errors?.lama || ''} />
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Input
                  ...
                  max={64}
                  value={maskDilatePx}
                  onChange={(e) => setMaskDilatePx(clamp(toNumber(e.target.value, 0), 0, 64))}
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  max={64}
                  value={maskFeatherPx}
                  onChange={(e) => setMaskFeatherPx(clamp(toNumber(e.target.value, 0), 0, 64))}
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  max={2048}
                  value={cropMarginPx}
                  onChange={(e) => setCropMarginPx(clamp(toNumber(e.target.value, 128), 0, 2048))}
                />
              </div>
            </div>
            ...
            {tool === 'remove_bg' && (
              <div className="mt-4 space-y-2">
                <Button variant="violet" onClick={runRemoveBg} disabled={!imageId || isRunning || !aiOk}>
                ...
                <Button
                  ...
                  disabled={!imageId || isRunning || !aiOk || !aiModels?.sam2}
                >
            ...
            {tool === 'erase' && (
              <div className="mt-4 space-y-3">
                ...
                <Button variant="violet" onClick={runErase} disabled={!imageId || isRunning || (quality === 'hq' ? hqOption.disabled : !aiOk)}>
          ...
          <div className="rounded-3xl border border-gray-200 bg-white shadow-sm p-4">
            <div className="flex items-center justify-between">
              ...
              {precisionMode && tool === 'remove_bg' && (
                <div className="text-xs font-semibold text-gray-500">Precision mode: click to add points</div>
              )}
            </div>
            ...
            <div className="mt-4 flex justify-center">
              {!imageSrc ? (
                ...
                <div ref={stageRef} className="relative inline-block select-none" onClick={addPointFromEvent}>
                  <img
                    ref={imgRef}
                    src={imageSrc}
                    alt="Uploaded"
                  ...
                  {/* Mask overlay (background removal) */}
                  {tool === 'remove_bg' && maskUrl && (
                    <img
                      src={maskUrl}
                      alt="Mask"
                      className="absolute inset-0 w-full h-full opacity-35 mix-blend-multiply pointer-events-none rounded-2xl"
                    />
                  )}

                  {/* SAM2 points */}
                  {tool === 'remove_bg' && precisionMode && points.map((p, idx) => (
                    <div
                      key={`${idx}-${p.label}`}
                      className={cn(
                  ...
                  {/* Mask paint canvas (eraser) */}
                  {tool === 'erase' && (
                    <canvas
                      ref={maskCanvasRef}
                      className="absolute inset-0 w-full h-full rounded-2xl"
                      style={{ cursor: eraseMode === 'erase' ? 'crosshair' : 'crosshair' }}
            ...
            <div className="mt-4 space-y-4">
                ...
                <>
                  <div>
                    ...
                    <div className="mt-2 rounded-2xl border border-gray-100 overflow-hidden" style={makeCheckerBg()}>
                      {cutoutUrl ? (
                        <img src={cutoutUrl} alt="Cutout" className="w-full h-auto block" />
                      ) : (
                        <div className="p-8 text-center text-sm text-gray-500">Run RMBG2 or SAM2 refine to preview.</div>
                      )}
                    </div>
                    {cutoutUrl && (
                      <div className="mt-2">
                  ...
                  <div>
                    ...
                    <div className="mt-2 rounded-2xl border border-gray-100 overflow-hidden bg-gray-50">
                      {maskUrl ? (
                        <img src={maskUrl} alt="Mask" className="w-full h-auto block" />
                      ) : (
                        <div className="p-8 text-center text-sm text-gray-500">Mask appears after background removal.</div>
                      )}
                    </div>
                    {maskUrl && (
                      <div className="mt-2">
              ...
              {tool === 'erase' && (
                <div>
                  ...
                  <div className="mt-2 rounded-2xl border border-gray-100 overflow-hidden bg-gray-50">
                    {eraseUrl ? (
                      <img src={eraseUrl} alt="Erase result" className="w-full h-auto block" />
                    ) : (
                      <div className="p-8 text-center text-sm text-gray-500">Paint a mask, then click Erase.</div>
                    )}
                  </div>
                  {eraseUrl && (
                    <div className="mt-2">
