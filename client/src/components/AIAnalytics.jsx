...
export default function AIAnalytics({ store, selectedStore, startDate, endDate }) {
            ...
            try {
              ...
              if (data.type === 'delta') {
                ...
                try {
                  ...
                  if (isValidChartPayload(payload)) {
                    const safeData = sanitizeChartData(payload.data, 'value');
                    setDockPayload({ ...payload, data: safeData });
  ...
  useEffect(() => {
    if (dockPinned || activeMode === 'explore') return;
        ...
        <div className="space-y-3">
          {Object.entries(modes).map(([modeKey, mode]) => (
            <div key={modeKey}>
              ...
              {activeMode === modeKey && modeKey !== 'explore' && (
