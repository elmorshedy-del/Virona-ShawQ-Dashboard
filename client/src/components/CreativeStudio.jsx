...
const addWavHeader = (pcmData, sampleRate = 24000, numChannels = 1, bitDepth = 16) => {
  ...
  const writeString = (viewInstance, offset, string) => {
    for (let i = 0; i < string.length; i += 1) {
      viewInstance.setUint8(offset + i, string.charCodeAt(i));
    }
  };
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              {['centered', 'split', 'framed'].map(l => (
                <button key={l} onClick={() => updateContent('layout', l)} className={`flex-1 py-1.5 text-[10px] uppercase tracking-wide border ${content.layout === l ? 'bg-black text-white border-black' : 'bg-white text-neutral-600 border-neutral-200'}`}>{l}</button>
              ))}
            </div>
          </div>
