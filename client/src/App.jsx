...
  const [manualOrders, setManualOrders] = useState([]);
    ...
    budgetIntelligence.liveGuidance.forEach(item => {
      ...
      if (!notifiedCampaignsRef.current.has(key)) {
        if (item.action === 'Scale') {
          addNotification('success', `🚀 Scale: ${item.campaignName}`, `${item.country} • ROAS ${item.roas?.toFixed(2)}x`);
          notifiedCampaignsRef.current.add(key);
        } else if (item.action === 'Cut') {
          addNotification('error', `🔻 Cut: ${item.campaignName}`, `${item.country} • ROAS ${item.roas?.toFixed(2)}x`);
          notifiedCampaignsRef.current.add(key);
        }
      }
    });
  ...
  useEffect(() => {
    notifiedCampaignsRef.current.clear();
...
                        notifications.map((notif) => (
                          <div key={notif?.id || Math.random()} className="p-3 border-b border-gray-50 last:border-b-0 hover:bg-gray-50">
                            ...
                            <p className="text-xs text-gray-500 mt-1">
                              ...
                              {notif?.details && <span> • {notif.details}</span>}
