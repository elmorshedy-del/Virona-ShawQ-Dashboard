...
def test_sync_meta_creatives_uses_asset_feed_spec_video_id(monkeypatch, tmp_path) -> None:
    db_path = tmp_path / "meta-creatives-asset-feed.db"
    settings = SimpleNamespace(
        meta_ready=True,
        meta_access_token="token",
        meta_ad_account_id="1234567890",
    ...
    assert len(records) == 1
    assert records[0].asset.asset_type == "video"
    assert records[0].asset.source_asset_id == "vid_asset_feed_1"
