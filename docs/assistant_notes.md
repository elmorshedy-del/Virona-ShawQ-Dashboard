...
- Branch hygiene rule (requested): always start from a clean branch/worktree and commit only Codex-authored changes.
...
- 2026-02-03: Dashboard (Creative section): added DeepSeek model selection + temperature for “Creative funnel AI summary” (stored in localStorage key `creativeInsightLlm` and sent via `llm` to `POST /api/ai/stream`).
- 2026-02-03: Attribution: removed accidental Session Intelligence “high-intent sessions” context injection from Attribution assistant (keeps Attribution focused; Session Intelligence owns Clarity-style brief).
- 2026-02-03: Meta Demographics: fixed Meta API error for `country+gender` by building `country×gender` via per-country gender calls (`breakdowns=gender` + `filtering` on country), then merging results for the UI. Adds `countryGenderSplitAvailable` and debug logging (`fbtrace_id`, filtering).
- 2026-02-03: Session Intelligence daily brief: hardened JSON parsing (code-fence stripping, balanced-brace extraction, trailing comma repair) and added an auto “JSON repair” retry + model fallback to reduce `AI response was not valid JSON` failures.
- 2026-02-03: Creative Intelligence: updated “Analyze with AI” to use Gemini 2.5 models (options: `gemini-2.5-flash-lite` / `gemini-2.5-flash`) and surfaced this as an “Analyze button (Gemini)” selector in Creative Intelligence AI Settings. Persists per-store in DB via `ai_creative_settings.gemini_analysis_model` and sends it to `POST /api/creative-intelligence/analyze-video`.
- 2026-02-03: Session Intelligence: added “Shop walk” funnel flow endpoint + UI (GA + Clarity style). Endpoint: `GET /api/session-intelligence/flow?store=...&date=YYYY-MM-DD&mode=all|high_intent_no_purchase`. UI renders per-step reached/conversion/dwell + drop-off clusters, and allows filtering the day sessions table by drop-off stage.
- 2026-02-03: Session Intelligence: added GA-style Realtime overview (last 30m) with backend aggregation endpoint `GET /api/session-intelligence/realtime?store=...&windowMinutes=30`. UI shows active sessions/shoppers, top pages/sources/events, and a live country hotspots map (Google GeoChart) with optional focus-on-top-country view.
- 2026-02-03: Session Intelligence Realtime polish: de-flickered the map by throttling redraws + only redrawing when geo distribution changes; normalized realtime event labels (Page Viewed / Product Viewed / Add to Cart / Checkout Started / Purchase), normalized traffic source labels (e.g., `ig`→Instagram, `meta`/`fb`→Facebook, `tt`→TikTok), and displayed country names (with ISO2 fallback) in the country breakdown.
- 2026-02-03: Session Intelligence: refactored key-event detection (ATC/Checkout/Purchase) to be centralized + pattern-tolerant (supports new/variant event names), and removed duplicated hardcoded event lists from SQL queries.
- 2026-02-04: Session Intelligence: added universal storefront tracker (`/pixel.js?store=...` → `/api/pixels/pixel.js`) capturing Clarity-style signals (rage clicks, dead clicks, scroll depth/max, JS errors, form validation friction). Added Clarity signals API (`GET /api/session-intelligence/clarity`) and UI section + session story drawer for examples.
- 2026-02-04: Session Intelligence: upgraded Clarity-style usability. Daily brief API now returns parsed `top_reasons` (not just `top_reasons_json`), the UI renders each reason with confidence + evidence + fixes, and provides one-click filtering to the likely drop-off stage. Shop-walk drop-off cluster chips are now clickable segment filters (device/country/campaign). Cart stage detection now also keys off `/cart` page paths (not just event names).
- 2026-02-03: Creative Analysis (Tab 11): restored the dedicated “Creative Preview” section (ad grid → modal preview → download MP4 / open on Facebook) via `client/src/components/CreativeAnalysis.jsx` + `client/src/components/CreativePreview.jsx` and removed the accidental duplicate Creative Intelligence UI living in those legacy files.
- 2026-02-03: Creative Intelligence chat: added multi-ad context selection (up to 5 analyzed ads) with “+ Compare/✓ In chat” toggles in the ad list and context chips above the chat input. Backend now accepts `adIds` in `POST /api/creative-intelligence/chat` and injects multiple ads’ Gemini analyses into the chat prompt for comparisons.
- 2026-02-04: Session Intelligence Realtime: normalized internal `si_*` event labels (`si_variant_changed` → “Variant Changed”, `si_atc_success` → “Add to Cart”) so they don’t show up as “Si …”.
- 2026-02-05: Session Intelligence: removed the “Purchases by campaign & country” section from the UI to reduce noise; converted the Sanity panel into a compact “Live events” pill; standardized session codenames to a cleaner format (`SXXXX-YYYY`) across payloads.
- 2026-02-05: Session Intelligence: replaced hashed session codenames with **per-store sequential session numbers** (`S-000123`). Added `si_sessions.session_number`, a per-store counter table `si_store_counters`, migration backfill for existing sessions, and joined `session_number` into events/sessions APIs so the UI can label sessions consistently.
- 2026-02-06: Session Intelligence v1 (Clarity-first layout): kept Realtime overview at the top, added a concise Summary strip + ranked Top Issues table (impact-ranked with normalized labels/actions), and moved heavy/legacy sections (shop walk, deep diagnostics, raw tables) under an explicit “Evidence + Advanced” toggle.
