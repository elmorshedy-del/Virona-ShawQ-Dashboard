...
- 2026-02-05: Universal pixel: reduced dead-click false positives by canceling dead clicks when a click produces a non-navigation “success signal” (meaningful scroll, `<details>`/accordion open-state change, `aria-expanded` toggle, label→checkbox/radio state change, or focus moved into an input).
