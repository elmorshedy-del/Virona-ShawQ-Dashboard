...
## January 19 Update (Old vs New)

| Area | Before | After |
| --- | --- | --- |
| Face detection | face-api.js (SSD Mobilenet) local models | SCRFD Python microservice + contour/geometry fallback |
| Renderer | Puppeteer HTML → PNG | Satori + Resvg SVG → PNG |
| Fonts | Remote/system fonts | Local fonts from server/assets/fonts (Inter + NotoColorEmoji) |
| Startup | Manual model readiness | Auto-start SCRFD service (disable with SCRFD_AUTO_START=0) |
| UI debug | No avatar sanity check | Avatar sanity check strip in Steps 2 & 3 |

Notes:
- SCRFD service lives in `server/scrfd/` and listens on `SCRFD_SERVICE_URL` (default: http://localhost:5050).
- If SCRFD is unavailable, extraction falls back to contour/geometry detection.
...
- Uses Gemini Vision API (`gemini-2.0-flash-exp`) for message extraction
...
- Uses SCRFD Python microservice for avatar detection (contour/geometry fallback)
...
- Uses Satori + Resvg to render SVG to PNG (local fonts)
...
- `twemoji` - Emoji parsing for SVG render
- `sharp` - Image processing and crops
...
- Satori + Resvg render clean, branded testimonial images
...
- testimonialRendererService.js (Satori + Resvg rendering)
...
<details>
<summary>Legacy Snapshot (Pre-Jan 19)</summary>
