...
function createMockServer() {
  const requestPathPattern = /^\/googleads\/v\d+\/customers\/1234567890\/googleAds:searchStream$/;
  ...
  return http.createServer((req, res) => {
    ...
    if (requestPathPattern.test(req.url || '') && req.method === 'POST') {
      let rawBody = '';
      req.on('data', (chunk) => {
...
async function run() {
  ...
  try {
    ...
    process.env.GOOGLE_ADS_API_BASE_URL = `http://127.0.0.1:${port}/googleads`;
