...
import { createOrderNotifications, createShopifyCampaignMatchNotifications } from './notificationService.js';
...
export async function syncMetaData(store) {
  ...
  try {
    ...
    if (store === 'shawq') {
      const matched = createShopifyCampaignMatchNotifications(store, { startDate, endDate });
      if (matched > 0) {
        console.log(`[Meta] Created ${matched} Shopify campaign match notifications for ${store}`);
      }
    }
