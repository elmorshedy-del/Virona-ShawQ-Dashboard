...
export function getBudgetIntelligence(store, params) {
  ...
  // Build country-isolated guidance for mixed-geo campaigns
  const countryAggregates = new Map();

  const addCountryAggregate = (row) => {
    if (!row.campaignId || !row.country) return;
    const key = `${row.campaignId}__${row.country}`;
    const startDate = row.startDate || null;
    const endDate = row.endDate || null;
    ...
    countryAggregates.set(key, existing);
  };

  campaignRows.forEach(addCountryAggregate);

  // Only use ad set rows for campaigns that lack campaign-level rows to avoid double counting
  adsetRows.forEach(row => {
    if (campaignLevelSeen.has(row.campaignId)) return;
    addCountryAggregate(row);
  });

  const campaignCountryGuidance = Array.from(countryAggregates.values()).map(row => {
    return processRowToGuidance({
      ...row,
      date: null,
      adsetId: null,
