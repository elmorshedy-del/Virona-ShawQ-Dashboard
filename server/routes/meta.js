...
function extractThumbnailUrl(creative) {
  ...
  const linkImage = creative?.object_story_spec?.link_data?.image_url
  ...
  if (Array.isArray(carouselElements) && carouselElements.length > 0) {
    ...
    if (firstElement?.picture) return firstElement.picture;
    if (firstElement?.image_url) return firstElement.image_url;
  }
...
router.get('/ads/:adId/video', async (req, res) => {
  ...
  const previewResult = await fetchMetaJson({
    path: `/${videoId}`,
    params: { fields: 'picture,thumbnails{uri},length,permalink_url,embed_html' },
    store,
    adAccountId,
    localEndpoint: '/api/meta/ads/:adId/video'
  });
  ...
  if (!sourceResult.ok) {
    if (sourceResult.data?.error?.code === 10) {
      return res.json({
        ...
        embed_html: previewData?.embed_html || null,
  ...
  res.json({
    ...
    embed_html: sourceData?.embed_html || previewData?.embed_html || null,
    thumbnail_url: thumbnailUrl,
    length: previewData?.length ?? null,
    permalink_url: previewData?.permalink_url || null
  });
