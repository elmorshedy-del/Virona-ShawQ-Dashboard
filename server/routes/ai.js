...
// ============================================================================
// ANALYTICS STREAM - Responses API + Tool Calling (Visualization Dock)
// ============================================================================
router.post('/analytics/stream', async (req, res) => {
  try {
    ...
    const onToolEvent = (event) => {
      res.write(`data: ${JSON.stringify({ type: 'tool', name: event.name, payload: event.payload })}\n\n`);
    };

    const result = await analyticsStreamWithTools(
      question,
      store,
      activeMode,
      depth || 'balanced',
