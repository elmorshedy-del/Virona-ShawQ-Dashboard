...
All images are passed as base64 (no data: prefix required).
"""
...
def require_ready(models: dict[str, bool]) -> tuple[bool, Any]:
    ...
    if all(models.values()):
        return True, None
    return False, jsonify({"error": "Models not ready (strict mode)", "models": models, "errors": {"rmbg2": RMBG2_ERROR, "sam2": SAM2_ERROR, "lama": LAMA_ERROR}})
...
def rmbg2_predict_mask(pil_image: Image.Image) -> Image.Image:
    ...
    with torch.inference_mode():
        ...
        # RMBG2 remote code returns a list/tuple; the last item is a 1x1xHxW mask logit.
...
def sam2_predict_mask_from_points(
    ...
    # Convert normalized points (0..1) to pixels.
    coords = []
    labels = []
    for p in points or []:
...
def remove_bg_rmbg2():
    try:
        ...
        # Downscale only for inference speed, but return mask at original resolution.
        w0, h0 = pil.size
        scale = 1.0
        ...
        if scale != 1.0:
            mask = mask_small.resize((w0, h0), resample=Image.BILINEAR)
        else:
            mask = mask_small
...
def remove_bg_sam2_refine():
    try:
        ...
        if feather_px:
            mask_u8_soft = feather_mask(mask_u8, feather_px)
        else:
            mask_u8_soft = mask_u8
...
def erase_lama():
    try:
        ...
        # Optional downscale for speed (keeps output size by compositing back at original size).
        scale = 1.0
        if max(w0, h0) > max_side:
            scale = max_side / float(max(w0, h0))
            pil_small = pil.resize((int(round(w0 * scale)), int(round(h0 * scale))), resample=Image.LANCZOS)
        ...
        # For blending we use a feathered alpha mask; LaMa itself thresholds >0 internally.
        ...
        if scale != 1.0:
            out_pil = out_pil_small.resize((w0, h0), resample=Image.LANCZOS)
        else:
